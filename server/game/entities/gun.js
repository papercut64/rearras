    ...
    recoil() {
        ...
        if (this.canShoot && !this.body.settings.hasNoRecoil) {
            ...
            if (this.motion > 0 || this.body.settings.hasNoReloadDelay) {
                ...
                // =======================================================
                // --- REDIRECT RECOIL TO THE SMOOTHING BUFFER QUEUE ---
                // =======================================================
                if (this.body.smoothedRecoil) {
                    // Feed the force to the multi-frame ease decay buffer
                    this.body.smoothedRecoil.x += recoilForce * Math.cos(this.recoilDir);
                    this.body.smoothedRecoil.y += recoilForce * Math.sin(this.recoilDir);
                } else {
                    // Fallback baseline safety: use raw instant acceleration if undefined
