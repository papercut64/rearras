class bulletEntity { // Basically an (Entity) but with heavy limitations to improve performance.
    constructor(position, master) {
        ...
        this.x = position.x;
        this.y = position.y;
        // --- CUSTOM BALLISTICS FRICTION PATCH ---
        this.age = 0;                  // Tracks how many server ticks the bullet has existed
        this.frictionDelay = 10;       // Default delay: Wait 10 server ticks (~0.33 second) before slowing down
        this.frictionAmount = 0.01;    // Default intensity: Loses 5% velocity per tick after the delay
        this.settings = {};
        this.aiSettings = {};
        this.guns = new Map();
        this.children = [];
    ...
    physics() {
        ...
        // ==========================================
        // --- CUSTOM BULLET FRICTION INJECTION ---
        // ==========================================
        // 1. Advance the bullet's age by 1 frame on every server tick
        this.age = (this.age || 0) + 1;

        // 2. Apply friction only after the bullet flies past your delay threshold
        // We use a fallback value (like 30 and 0.05) if a gun doesn't have custom values in gunvals.js
        let delay = this.frictionDelay ?? 30;
        let amount = this.frictionAmount ?? 0.05;

        if (this.age > delay) {
            this.velocity.x *= (1 - amount);
            this.velocity.y *= (1 - amount);
            
            // Performance Safeguard: If friction stops the bullet entirely, kill it to save server memory
            if (Math.abs(this.velocity.x) < 0.05 && Math.abs(this.velocity.y) < 0.05) {
                this.destroy(); // Safely deletes the entity using the native cleanup method
                return;         // Stop processing the rest of this function since the bullet is dead
            }
        }
        // ==========================================
