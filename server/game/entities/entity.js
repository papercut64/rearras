...
class Entity extends EventEmitter {
    constructor(position, master) {
        ...
        this.damp = 0.05;
        // --- CUSTOM SMOOTHED RECOIL STORAGE ---
        this.smoothedRecoil = { x: 0, y: 0 };
        this.recoilSmoothingFactor = 0.20;
    ...
    physics() {
        ...
        // ==========================================
        // --- CUSTOM SMOOTHED RECOIL INJECTION ---
        // ==========================================
        if (this.smoothedRecoil) {
            // Calculate a localized portion of the queued recoil to apply this frame tick
            let stepX = this.smoothedRecoil.x * this.recoilSmoothingFactor;
            let stepY = this.smoothedRecoil.y * this.recoilSmoothingFactor;

            // Apply that fractional portion to the entity velocity vector
            this.velocity.x += stepX;
            this.velocity.y += stepY;

            // Deduct what was applied from the queue so it decays down over time
            this.smoothedRecoil.x -= stepX;
            this.smoothedRecoil.y -= stepY;

            // Prevent floating point noise from running infinitely
            if (Math.abs(this.smoothedRecoil.x) < 0.01) this.smoothedRecoil.x = 0;
            if (Math.abs(this.smoothedRecoil.y) < 0.01) this.smoothedRecoil.y = 0;
        }
        // ==========================================
